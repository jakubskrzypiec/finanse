...
const progress=$('.scroll-progress span');
  ...
  // v18 — slower, RAF-throttled cinematic optimized for lower-power hardware.
  ...
  function renderCinematic(){
    ...
    // Very calm opening. Only transform + opacity are animated.
    const zoom=smoother(clamp01((p-.08)/.72));
    const targetScale=geom
      ? Math.max(innerWidth/geom.screenW,innerHeight/geom.screenH)*1.012
      : 4.05;
    ...
    laptop.style.transform =
      `translate3d(-50%,-50%,0) translate3d(${dx}px,${dy}px,0) scale(${scale})`;

    // Logo disappears slowly and earlier.
    const logoFade=easeOut(clamp01((p-.43)/.20));
    if(screenBrand){
      screenBrand.style.setProperty('--screen-logo-opacity',String(1-logoFade));
    }

    // Screen softly becomes the hero background.
    const wash=easeOut(clamp01((p-.54)/.20));
    if(screenBrand){
      screenBrand.style.setProperty('--screen-wash',String(wash));
    }

    // Hardware dissolves near the end. Only opacity is changed for child parts;
    // expensive dynamic shadows/background calculations are avoided.
    const hardware=smooth(clamp01((p-.69)/.14));
    if(display) display.style.opacity=String(1-hardware);
    ...
    if(shadow) shadow.style.opacity=String((1-hardware)*.13);

    // Direct real-hero reveal.
    const reveal=easeOut(clamp01((p-.76)/.22));
  ...
  function requestCinematicFrame(){
    if(!rafId) rafId=requestAnimationFrame(renderCinematic);
  }
  ...
  if(cinematic){
    ...
    addEventListener('scroll',requestCinematicFrame,{passive:true});
    ...
    addEventListener('resize',()=>{
      ...
      resizeTimer=setTimeout(()=>{
        ...
        requestAnimationFrame(()=>{
          ...
          requestCinematicFrame();
        });
      },120);
    },{passive:true});

    // Release compositor hints after the intro to reduce memory pressure.
    cinematic.addEventListener('transitionend',()=>{}, {once:true});
  } else {
    $('.hero-after-cinematic')?.style.setProperty('--hero-reveal','1');
  }
