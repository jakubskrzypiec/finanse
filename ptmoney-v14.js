...
const progress=$('.scroll-progress span');
  ...
  function updateCinematic(){
    ...
    // As the screen fills the viewport, fade the logo into the exact cream used by the hero.
    const wash=easeOut(clamp01((p-.62)/.17));
    if(screenBrand){
      screenBrand.style.setProperty('--screen-wash',String(wash));
    }

    // The physical laptop quietly dissolves after the screen already dominates the frame.
    const hardware=smooth(clamp01((p-.69)/.13));
    if(display){
    ...
    if(p>.87) document.body.classList.add('cinema-site-ready');
