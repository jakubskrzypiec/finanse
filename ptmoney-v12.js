...
const progress=$('.scroll-progress span');
  ...
  // v12 — refined 3-stage cinematic laptop entrance.
  ...
  function updateCinematic(){
    ...
    // Stage 1 — almost static establishing shot.
    const establish=smoother(clamp01((p-.02)/.28));
    world.style.transform=`scale(${1+establish*.018})`;

    // Stage 2 — screen content wakes up before the camera moves aggressively.
    const portal=easeOut(clamp01((p-.18)/.20));
    if(screenBrand){
      screenBrand.style.opacity=String(1-portal);
      screenBrand.style.transform=`scale(${1-portal*.015})`;
    }
    if(screenPortal){
      screenPortal.style.opacity=String(portal);
      screenPortal.style.transform=`scale(${.99+portal*.01})`;
    }

    // Stage 3 — dolly into the screen, slow at first and faster near the end.
    const zoom=smoother(clamp01((p-.27)/.57));
    const targetScale=geom
      ? Math.max(innerWidth/geom.screenW,innerHeight/geom.screenH)*1.02
      : 4.2;
    ...
    // Hardware dissolves only when screen is already almost filling the viewport.
    const hardware=smooth(clamp01((p-.68)/.15));
    if(display){
      display.style.background=`rgba(145,148,144,${1-hardware})`;
      display.style.boxShadow=`0 20px 42px rgba(0,0,0,${.25*(1-hardware)})`;
    ...
    // Direct reveal of the REAL hero underneath — no extra green screen.
    const reveal=easeOut(clamp01((p-.80)/.18));
