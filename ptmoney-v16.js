...
const progress=$('.scroll-progress span');
  ...
  // v14 — clean laptop entrance: logo -> zoom -> real hero.
  ...
  function updateCinematic(){
    ...
    // Very subtle camera drift at the beginning.
    const room=smoother(clamp01((p-.02)/.30));
    world.style.transform=`scale(${1+room*.012})`;

    // One continuous dolly into the laptop screen.
    const zoom=smoother(clamp01((p-.13)/.66));
    const targetScale=geom
      ? Math.max(innerWidth/geom.screenW,innerHeight/geom.screenH)*1.025
      : 4.2;
    ...
    // Clean handoff: the logo disappears first, then the screen becomes the hero background.
    const logoFade=easeOut(clamp01((p-.50)/.16));
    const wash=easeOut(clamp01((p-.57)/.18));
    if(screenBrand){
      screenBrand.style.setProperty('--screen-logo-opacity',String(1-logoFade));
      screenBrand.style.setProperty('--screen-wash',String(wash));
    }

    // The physical laptop quietly dissolves only after the screen dominates the viewport.
    const hardware=smooth(clamp01((p-.68)/.13));
    if(display){
    ...
    // The real hero has been under the final viewport the whole time.
    // Reveal the REAL hero directly — no stop on the laptop logo and no extra screen.
    const reveal=easeOut(clamp01((p-.75)/.23));
